...
public class SessionAssistant : MonoBehaviour {
	...
	List<Chip> FindPowerups ()
	{
        ...
        CrossBomb cross;
        ColorBomb color;
        RainbowHeart lightning;
        Ladybird ladybird;
        foreach (Chip bomb in GameObject.FindObjectsOfType<Chip>()) {
            simple = bomb.gameObject.GetComponent<SimpleBomb>();
            if (simple) {
                pu.Add(bomb);
                continue;
            }
            cross = bomb.gameObject.GetComponent<CrossBomb>();
            if (cross) {
                pu.Add(bomb);
                continue;
            }
            color = bomb.gameObject.GetComponent<ColorBomb>();
            if (color) {
                pu.Add(bomb);
                continue;
            }
            lightning = bomb.gameObject.GetComponent<RainbowHeart>();
            if (lightning) {
                pu.Add(bomb);
                continue;
            }
            ladybird = bomb.gameObject.GetComponent<Ladybird>();
            if (ladybird) {
                pu.Add(bomb);
                continue;
            }
